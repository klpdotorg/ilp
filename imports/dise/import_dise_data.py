...
def create_sqlfiles():
    ...
    for table in tables:
        for year in years:
            ...
            system('echo "'+table['query'].replace('replacecolumns',table['columns']).replace('replacefile',table['name']).replace('replaceyear',year)+'\">>'+inputdatafile)
            ...
            system('echo "COPY '+table['name']+"("+table['fixedcolumns']+","+table['columns']+") from '"+filename+"' with csv NULL 'null';"+'\">>'+loaddatafile)
