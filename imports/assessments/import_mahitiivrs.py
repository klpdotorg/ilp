...
if len(sys.argv) != 3:
    print("Please give database names as arguments. USAGE: " +
          "python import_mahitiivrs.py dubdubdub ilp")
    sys.exit()
...
tables = [
    {
        ...
        'getquery': "COPY(select id, trim(name), created_at, updated_at, 'akshara', 'AC' from stories_survey where id=6) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    ...
    },
    {
        ...
        'getquery': "COPY(select id, 'Mahiti IVRS', to_date('2014-04-03', 'YYYY-MM-DD'), version, 0, to_date('2014-04-03', 'YYYY-MM-DD'), case(school_type_id) when 1 then 'primary' when 2 then 'pre' else 'both' end, case(status) when 2 then 'AC' when 1 then 'IA' else 'AC' end,    source_id, survey_id, 'institution', 'monitor', 'name' from stories_questiongroup where survey_id=6) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
        ...
        #'getquery': "COPY(select id, 'Mahiti IVRS', to_date('2014-04-03', 'YYYY-MM-DD'), version, 0, to_date('2014-04-03', 'YYYY-MM-DD'), case(school_type_id) when 1 then 'primary' when 2 then 'pre' else 'both' end, case(status) when 2 then 'AC' when 1 then 'IA' else 'AC' end,    source_id, survey_id, 'institution', 'monitor', 'name', created_by_id from stories_questiongroup where survey_id=6) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
        #'insertquery': "COPY replacetablename(id, name, start_date, version, double_entry, created_at, inst_type_id, status_id, source_id,survey_id, survey_on_id, type_id, group_text, created_by_id) FROM 'replacefilename' with csv NULL 'null';"
    },
    {
    ...
    },
    {
        ...
        'getquery': "COPY(select questiongroup_id, question_id, sequence from stories_questiongroup_questions where questiongroup_id in (select id from stories_questiongroup where survey_id =6)) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    ...
    },
    {
        ...
        'getquery': "COPY(select distinct stories.id, 0, stories.date_of_visit, stories.comments, stories.is_verified, stories.sysid, stories.entered_timestamp, stories.school_id, stories.group_id, case qg.status when 1 then 'IA' when 2 then 'AC' else 'AC' end, usertype.name, stories.name from stories_story stories, stories_questiongroup qg, stories_usertype usertype where stories.group_id = qg.id and qg.survey_id=6 and stories.user_type_id = usertype.id) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
        ...
        #'getquery': "COPY(select distinct stories.id, 0, stories.date_of_visit, stories.comments, stories.is_verified, stories.sysid, stories.entered_timestamp, stories.school_id, stories.group_id, case qg.status when 1 then 'IA' when 2 then 'AC' else 'AC' end, usertype.name, stories.name, stories.user_id from stories_story stories, stories_questiongroup qg, stories_usertype usertype where stories.group_id = qg.id and qg.survey_id=6 and stories.user_type_id = usertype.id) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    ...
    },
    {
        ...
        'getquery': "COPY(select story_id, question_id, text from stories_answer answer, stories_story stories where answer.story_id=stories.id and stories.group_id in (select id from stories_questiongroup where survey_id =6)) TO 'replacefilename' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
