...
if len(sys.argv) != 3:
    print("Please give database names as arguments. USAGE: python import_studentgrouprelations_data.py ems ilp", file=sys.stderr)
    sys.exit()
...
tables=[
    {
        ...
        'tempquery': "COPY(select st.id, concat(substr(split_part(ay.name, '-',1),3,4), substr(split_part(ay.name,'-',2),3,4)), st.staff_id, case st.active when 2 then 'AC' when 1 then 'IA' when 0 then 'DEL' end, st.student_group_id from schools_staff_studentgrouprelation st, schools_academic_year ay where st.academic_id=ay.id) TO '$PWD/load/schools_staffstudentgrouprelation.csv' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
