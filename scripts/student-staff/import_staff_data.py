...
if len(sys.argv) != 3:
    print("Please give database names as arguments. USAGE: python import_staff_data.py ems ilp", file=sys.stderr)
    sys.exit()
...
tables=[
    {
        ...
        'columns': 'id, name',
        'query': "COPY(select id, qualification from schools_staff_qualifications) TO '$PWD/load/schools_qualification.csv' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    },
    {
        'name': 'schools_stafftype',
        'table_name': 'schools_stafftype',
        'columns': 'id, staff_type, institution_type_id',
        'query': "COPY(select id, staff_type, case category_type when 1 then 'primary' when 2 then 'pre' end from schools_staff_type) TO '$PWD/load/schools_stafftype.csv' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    },
    {
        'name': 'schools_staff',
        ...
        'query': "COPY(select id, first_name, middle_name, last_name, uid, doj, gender, institution_id, case mt_id "+mt_case+", staff_type_id, 'AC' from schools_staff where active=2 and institution_id in (select id from schools_institution where active=2)) TO '$PWD/load/schools_staff.csv' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    },
    {
        'name': 'schools_staffqualification',
        'table_name': 'schools_staffqualification',
        'columns': 'id, qualification_id, staff_id',
        'query': "COPY(select sq.id, sq.staff_qualifications_id, sq.staff_id from schools_staff_qualification sq, schools_staff s where sq.staff_id=s.id and s.active=2 and s.institution_id in (select id from schools_institution where active=2))  TO '$PWD/load/schools_staffqualification.csv' NULL 'null' DELIMITER ',' quote '\\\"' csv;",
    }
]
