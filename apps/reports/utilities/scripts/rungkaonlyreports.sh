...
python manage.py sendreports DistrictReport --gka=True --gp=False --hhsurvey=False --from=$FROM --to=$TO "$CSV_DIR_PATH/1million_districts.csv"
echo "Done sending reports"
TIMESTAMP=`date "+%Y-%m-%d %H:%M:%S"`
echo $TIMESTAMP      
echo "================================"
echo "Starting block  reports run `date '+%Y-%m-%d %H:%M:%S'`"
python manage.py sendreports BlockReport --gka=True --gp=False --hhsurvey=False --from=$FROM --to=$TO "$CSV_DIR_PATH/1million_blocks.csv"
echo "Done sending reports"
TIMESTAMP=`date "+%Y-%m-%d %H:%M:%S"`
echo $TIMESTAMP
echo "================================"
echo "Starting cluster  reports run `date '+%Y-%m-%d %H:%M:%S'`"
python manage.py sendreports ClusterReport --gka=True --gp=False --hhsurvey=False --from=$FROM --to=$TO "$CSV_DIR_PATH/1million_clusters.csv"
echo "Done sending reports"
TIMESTAMP=`date "+%Y-%m-%d %H:%M:%S"`
echo $TIMESTAMP      
echo "================================"
