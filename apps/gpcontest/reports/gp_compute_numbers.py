...
from django.db.models import (
    When,
    Case, Value, Sum, Avg, F, ExpressionWrapper, Count)
...
def get_grade_competency_correctscores(gp_id, qgroup_id, gpcontest_survey_id,
    ...
    try:
        ...
        print("correct_answers_agg", SurveyEBoundaryQuestionGroupQuestionKeyCorrectAnsAgg.objects
              .filter(survey_id=gpcontest_survey_id,
                      eboundary_id=gp_id, survey_tag='gka').annotate(Avg('average')))
    except SurveyEBoundaryQuestionGroupQuestionKeyCorrectAnsAgg.DoesNotExist:
        pass
    return correct_answers_agg
...
def get_grade_competency_percentages(gp_id, qgroup_id, gpcontest_survey_id,
    ...
    if total_assessments is not None and correct_answers_agg is not None:
        for contest_date in contest_dates:
            ...
            competency_order = CompetencyOrder.objects.filter(questiongroup=qgroup_id).order_by('sequence').values_list(
                'key', flat=True)
            ...
            for each_row in total_ans_for_date:
                print("each_row in percentage", each_row)
                ...
                try:
                    ...
                    print("each_row in percentage correct answer", sum_correct_ans)
                except:
                ...
                if sum_total is not None:
                    percentage = round(sum_correct_ans, 2)  # round((sum_correct_ans / sum_total)*100, 2)
                else:
                    percentage = 0
...
def getCompetencyPercPerSchool(survey_id, school_id, key, from_yearmonth, to_yearmonth):
    ...
    correct_ans = SurveyInstitutionQuestionGroupQuestionKeyCorrectAnsAgg.objects.filter(
        ...
        institution_id=school_id).filter(yearmonth__gte=from_yearmonth).filter(
        yearmonth__lte=to_yearmonth).filter(
        question_key=key).values('survey_id', 'institution_id', 'question_key').annotate(
        total_answers=Sum('numcorrect'))
    correct = 0
    if correct_ans:
...
'''
Used by household reports
'''


def getGPCompetencyPercForHousehold(survey_id, gp_id, key, from_yearmonth, to_yearmonth):
    """ For household reports, that need addition/subtraction percentages
        per school """

    correct_ans = SurveyEBoundaryQuestionGroupQuestionKeyCorrectAnsAgg.objects.filter(
        survey_id=survey_id).filter(
        eboundary_id=gp_id).filter(yearmonth__gte=from_yearmonth).filter(
        yearmonth__lte=to_yearmonth).filter(
        question_key=key).values('survey_id', 'eboundary_id', 'question_key').annotate(total_answers=Sum('numtotal'),
                                                                                       correct=Sum('numcorrect'))
    correct = 0
    correct_answer = None
    total = None
    if correct_ans:
    ...
    if correct_answer and correct_answer['total_answers'] is not None:
        total = correct_answer['total_answers']
        correct = correct_answer["correct"]
    if total is None:
        # Data unavailable for this GP for this competency
        perc = 'NA'
    elif total > 0:
