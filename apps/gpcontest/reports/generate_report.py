...
def generate_gp_summary(gp_id, gp_survey_id, from_yearmonth, to_yearmonth):
    ...
    if schools.count() > 0:
        district_name = Boundary.objects.get(id=schools.first().admin1_id).name.capitalize()
        district_lang_name = Boundary.objects.get(id=schools.first().admin1_id).lang_name
        block_name = Boundary.objects.get(id=schools.first().admin2_id).name.capitalize()
    ...
    for index, date in enumerate(yearmonth_dates):
        ...
        all_scores_for_gp = {
            "gp_name": gp_name,
            "gp_lang_name": gp_lang_name,
            "gp_id": gp_id,
            "district": district_name,
            "district_lang_name": district_lang_name,
            "block": block_name,
            "block_lang_name": block_lang_name,
