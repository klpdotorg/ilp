...
def generate_gp_summary(gp_id, gp_survey_id, from_yearmonth, to_yearmonth):
    ...
    try:
        ...
        gp_display_name = gp.const_ward_name.capitalize()
        if gp.const_ward_lang_name and gp.const_ward_lang_name is not " ":
    ...
    if schools.count() > 0:
        ...
        district_display_name = district_name.capitalize()
        # If the local lang name is available, set the display name to be that
        if district_lang_name:
            district_display_name = district_lang_name + " (" + district_name.capitalize() + ")"
        block_name = Boundary.objects.get(id=schools.first().admin2_id).name
        block_display_name = block_name.capitalize()
        block_lang_name = Boundary.objects.get(id=schools.first().admin2_id).lang_name
        if block_lang_name:
            block_display_name = block_lang_name + " (" + block_name.capitalize() + ")"
    ...
    for index, date in enumerate(yearmonth_dates):
        ...
        all_scores_for_gp = {
            "gp_name": gp_name,
            "gp_display_name": gp_display_name,
            "gp_id": gp_id,
