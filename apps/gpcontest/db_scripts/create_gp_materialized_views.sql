...
CREATE MATERIALIZED VIEW mvw_gpcontest_eboundary_answers_agg AS
    ...
    SELECT id, gp_id, questiongroup_id, yearmonth,
                    ...
                    COUNT(1) FILTER (WHERE ROUND(total_percent,2)>=35.00 AND ROUND(total_percent,2)<61.00) as cat_b,
...
CREATE MATERIALIZED VIEW mvw_gpcontest_boundary_answers_agg AS
    ...
    SELECT id, boundary_id, boundary_type_id, questiongroup_name, boundary_name,
                    COUNT(*) as num_students,
                    ...
                    COUNT(1) FILTER (WHERE ROUND(total_percent,2)>75.00 AND ROUND(total_percent,2)<101.00) as cat_d
    FROM subquery1
    GROUP BY id,boundary_id, boundary_type_id, questiongroup_name,boundary_name;
