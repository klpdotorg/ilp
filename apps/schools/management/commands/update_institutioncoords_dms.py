...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for row in file_reader:
            ...
            latitudeDeg = self.getValidFloatValues(row[1].strip())
            latitudeMin = self.getValidFloatValues(row[2].strip())
            latitudeSec = self.getValidFloatValues(row[3].strip())
            longitudeDeg = self.getValidFloatValues(row[4].strip())
            longitudeMin = self.getValidFloatValues(row[5].strip())
            longitudeSec = self.getValidFloatValues(row[6].strip())
